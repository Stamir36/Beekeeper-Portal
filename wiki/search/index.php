...
//Loader data
  ...
  if(isset($_GET["search"])){
    $value_search = urldecode($_GET["search"]); // Получение результата поиска
  }
  if(strlen($value_search) <= 0){
    header('Location: /wiki/');
  }

  $result_count = $mysql->query("SELECT COUNT(*) FROM `wiki` WHERE header LIKE '%$value_search%'");
    $count_num = $result_count->fetch_assoc();
    $count = $count_num['COUNT(*)']; // Количество записей

    if($page <= 0){
      header('Location: ?search='.$value_search.'&page=1');
    }
...
<html lang="ua">
  ...
  <header class="header">
    ...
    <section class="position-relative py-6"><img src="https://www.shutterstock.com/blog/wp-content/uploads/sites/5/2018/04/Gradient-Featured-Image-03.jpg" alt="" class="bg-image">
      <div class="container">
        <div class="row">
          <div class="col-lg-6">
            <div class="bg-white rounded-lg shadow p-5"><strong class="text-uppercase text-secondary d-inline-block mb-2 text-sm">Пошук</strong>
              <h2 class="mb-3"><?php echo($value_search); ?></h2>
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="py-6">
      <div class="container">
        ...
        <?php
        $wiki = $mysql->query("SELECT * FROM `wiki` WHERE header LIKE '%$value_search%' AND body LIKE '%$value_search%' limit 10 OFFSET $OFFSET");
        $category = array();
        $id_wiki = array();
        $data = array();
        $photo = array();
        ...
        if (count($photo) == count($name) && count($data) != 0 && count($name) != 0) {
          ...
          echo ("<a class='t_mobile' style='color: #322EFF; text-align: center; background-color: #fff; padding: 10px; border-radius: 4px;'>Нічого не знайдено 😅</a>");
        }
        ?>
