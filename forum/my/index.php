<?php
    ...
    $result_count = $mysql->query("SELECT COUNT(*) FROM `forum_main_mess` WHERE author_id = '$cook_id'");
    ...
    $theme = $mysql->query("SELECT `forum_main_mess`.`id`, `forum_main_mess`.`header` FROM `forum_main_mess` WHERE `forum_main_mess`.`author_id` = '$cook_id' ORDER BY `forum_main_mess`.`Date_Create` ASC limit 10 OFFSET $OFFSET");
...
<html>
    <head>
        ...
        <title>Мої теми - форум пасічників</title>
            ...
            <article class="main" style="height: 100px;">
                <div class="firstcoll" style="height: 100px;">
                  <div style="height: 100px; padding: 15px;">
                    <h1 class="main_text" style="color: #fff; font-weight: 500;">Мої теми 💬</h1>
                  </div>
              </div>
            </article>
            ...
            <?php             
              if(strlen($cook_id) > 0){
                ...
                <div style='padding-left: 10px;'>
                  ...
                  <a href='../liked/' class='btn btn-warning'> Мої обрані теми </a>
                </div>
                ");
              }
            ?>
                            ...
                            <div class="widget-content widget-content-area br-6">
                                <div class="table-responsive mb-4 mt-4" style="background-color: rgb(255, 255, 255); padding: 5px; border-radius: 5px;">
                                          ...
                                          <?php                                            
                                            ...
                                            $num_count = 0;
                                            while($num_count <= (count($id_theme) - 1)){
                                              $count_varible = $mysql->query("SELECT COUNT(*) FROM `forum_sub_mess` WHERE `id_main_mess` = '$id_theme[$num_count]'");
                                              $var = $count_varible->fetch_assoc();

                                              $count_mess[$num_count] = $var['COUNT(*)'];
                                            ...
                                            if(count($id_theme) == count($names) && count($id_theme) != 0 && count($names) != 0 ){
                                              ...
                                              <thead>
                                                  <tr>
                                                      <th>Назва теми</th>
                                                      <th class='no_mobile'>Повідомлень</th>
                                              ...
                                              while($num_theme <= (count($id_theme) - 1)){
                                                ...
                                                <div class='theme_point'>
                                                  <tr  onclick='go_href(".$id_theme[$num_theme].")' style='cursor: pointer;'>
                                                    ...
                                                    <td class='no_mobile'>Всього: ".$count_mess[$num_theme]."</td>
                                                    <td></td>
                                                  </tr>
                                                </div>
                                                ");
                                              ...
                                              <div style='margin: 10px;'>
                                                <a style='color:red; padding: 10px;'>Ви не створили жодної теми.</a>
                                              </div>
                                              ");
                                            }
